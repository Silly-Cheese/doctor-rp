...
function ensureStyles(){if(document.querySelector('link[href^="northstar-enterprise.css"]'))return;const l=document.createElement("link");l.rel="stylesheet";l.href="northstar-enterprise.css?v=20260917-2210";document.head.appendChild(l);}
...
function openForm(title,sub,body,handler){const d=document.querySelector("#enterpriseFormDialog"),f=document.querySelector("#enterpriseForm"),b=document.querySelector("#enterpriseFormBody"),btn=document.querySelector("#enterpriseFormSubmit");const labels={"Procedure & Consent":"Create Procedure","Procedure Safety Timeout":"Complete Timeout","Complete Procedure":"Complete Procedure","Inpatient Admission":"Admit Patient","Discharge Plan":"Save Discharge Plan","Incoming EMS":"Create EMS Report","Blood Bank Request":"Create Request","Supply Item":"Add Supply"};const submitLabel=labels[title]||"Save";document.querySelector("#enterpriseFormTitle").textContent=title;document.querySelector("#enterpriseFormEyebrow").textContent=sub;b.innerHTML=body;btn.textContent=submitLabel;f.onsubmit=async e=>{e.preventDefault();btn.disabled=true;btn.textContent="Saving…";try{await handler();d.close();toast(`${title} saved.`)}catch(error){console.error("Northstar workflow save failed",error);toast("Unable to save this workflow.")}finally{btn.disabled=false;btn.textContent=submitLabel}};d.showModal();}
